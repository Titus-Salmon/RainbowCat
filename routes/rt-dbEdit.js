...
router.post('/results', (req, res, next) => { //take POST request data from dbEdit page & put into database table
	...
	console.log('formInput3(from dbEdit)==>', formInput3);
	console.log('formInput4(from dbEdit)==>', formInput4);
...
router.post('/saveCSV', (req, res, next) => {
	...
	res.render('vw-csvSaved', { //render searchResults to vw-dbEdit page
		title: 'CSV Saved'
	});
})

router.post('/deleteSelection', (req, res, next) => { //take POST request data from vw-dbEdit page & delete from database table
	const postBody = req.body;
	console.log('postBody', postBody);
